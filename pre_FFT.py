...
def FFT(img):
    ...
    # 转换为图像
    print(type(srcIm.mode))  # <class 'str'>
    print(type(srcIm.size))  # <class 'tuple'>
    im = Image.frombytes(srcIm.mode, srcIm.size, result)
    # im = Image.frombytes('RGB', (srcIm.shape[1], srcIm.shape[0]), result)
    im.show()
    im.save('C:\\Users\\chenjinjin\\Desktop\\test_image\\50_ufft_#.jpg')
